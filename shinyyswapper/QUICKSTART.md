...
**Cell 3: Setup Kaggle API** (ONE TIME ONLY)
```python
...
```
- Click ▶️ Run
- Click "Choose Files" and upload `kaggle.json`
- **Get kaggle.json:** Go to kaggle.com → Sign in → Settings → API → Create New Token

**Cell 4: Download 80K Images** (AUTOMATIC!)
```python
# Downloads FFHQ + CelebA-HQ from Kaggle
```
- Click ▶️ Run
- Wait 30-60 minutes (downloads 80K images to your Drive)
- ☕ Take a break!

**Cell 5: Clone Your Code**
```python
...
3. **Run cells 1, 2, 4, 5, 6, 7** (SKIP Cell 3 - Kaggle already configured)
