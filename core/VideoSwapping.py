...
import shutil
import time
from core.SinglePhoto import FaceSwapper
...
def main():
    ...
    stabilize = input("Enable video stabilization? (y/n, default=y): ").strip().lower()
    use_stabilization = stabilize != 'n'
    ...
    if choice == "2" or choice == "3":
        ...
        # Initialize face swapper and stabilizer
        swapper = FaceSwapper()
        ...
        for idx, frame_path in enumerate(frame_paths):
            ...
            if os.path.exists(out_path):
                ...
                prev_frame = cv2.imread(out_path)
                continue
            try:
                try:
                ...
                if use_stabilization and stabilizer is not None:
                    # Get face bbox for stabilization
                    frame = cv2.imread(frame_path)
                    ...
                    faces = app.get(frame)
                    if faces:
                        face = sorted(faces, key=lambda x: x.bbox[0])[min(dest_face_idx-1, len(faces)-1)]
